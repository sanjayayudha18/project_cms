...
### VendorPortal-Vite: `configureApiAuth` must actually be called, or every `api.*` request 401s silently
`frontend/VendorPortal-Vite/src/lib/api/client.ts` exposes `configureApiAuth({ getToken, refresh, onFailure })` to wire the API client to the auth context, but nothing ever called it — `getAccessToken`/`onRefreshToken`/`onAuthFailure` stayed `null` forever, so `injectAuthHeader` never attached an `Authorization` header on any request through `api.get/post/put/patch/delete`. Symptom: the DSR upload dialog showed "Unauthorized" on every attempt, even immediately after a fresh login — because `AuthContext.tsx`'s `login()`/`refreshToken()` use raw `fetch()` directly (so login itself succeeded and set React state), but that state was never propagated to the module-level client. Logout/login-again looked like it should fix a stale-token issue but didn't, because the bug wasn't token staleness — it was a missing wire.
Fix: in `AuthContext.tsx`, keep a `useRef` synced to `accessToken` via `useEffect`, and call `configureApiAuth({ getToken: () => accessTokenRef.current, refresh: refreshToken, onFailure: ... })` once in a `useEffect([refreshToken])`. Use a ref (not the raw state value) for `getToken` so the closure doesn't go stale after subsequent logins/refreshes.
**Diagnostic pattern for "Unauthorized despite fresh login" in this app**: check whether the failing call goes through `api.*` (client.ts) vs a direct `fetch()` in `AuthContext.tsx` — if `configureApiAuth` was never called, only `api.*` calls are broken, direct-fetch auth calls (login/logout/refresh) work fine, which is a strong tell it's a wiring gap, not a token-expiry issue.
