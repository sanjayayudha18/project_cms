...
### `ponytail:` is the project's custom TODO marker, NOT a typo — do not "fix" it
Across specs and `backend/cmd/api/main.go`, comments like `ponytail: swap dbPool for the dbRead pool when DATABASE_REPLICA_URL wiring lands` use `ponytail:` as a deliberate, greppable TODO convention (primarily marking read-replica-pool swaps that are pending `DATABASE_REPLICA_URL` wiring). It looks like a corrupted autocomplete artifact and reads like it should say `replica:` or `TODO:`, but it is intentional and consistent. Before editing anything that looks like a typo in this codebase, grep for it (`grep_search "ponytail"`) — if it recurs in real source under `backend/`, it's a convention, not an error. Leave it alone and match it when adding new replica-swap TODOs.

### When asked to "fix errors" in a spec with no build step, look for stale cross-references
Spec docs (`.kiro/specs/*/{requirements,design,tasks}.md`) have no compiler, so "errors" usually mean internal inconsistencies from edits that renamed things without updating every reference. Common ones seen in `admin-user-vendor-management`: a task bullet referencing a phantom task id ("If Task 0a guards..." when no Task 0a exists), and stale terminology ("Open Decision 1" after the section was renamed to "Resolved Decisions"). Grep the whole spec folder for the old term/id after any rename. Requirement sub-numbers (e.g. `14.5`) render as bare list markers (`5.`), so a `grep "14.5"` returns nothing even when the criterion exists — count list items under the requirement heading instead of trusting the grep miss.

### Kiro spec `design.md` requires exact top-level section headings — the validator matches on heading text
The Kiro spec validator flags `design.md` with errors like "Missing required section: ## Components and Interfaces" when a `##`-level heading with that exact name is absent. It matches on the literal heading text at `##` level, not on whether the content exists. In `admin-user-vendor-management`, the content lived under `## Backend components` / `## Frontend components`, which the validator did not accept. Fix: add the canonical `## Components and Interfaces` heading and nest the existing groups beneath it (`### Backend components` / `### Frontend components`, their subsections demoted to `####`) — no content rewrite needed, just re-level the headings so the required `##` name is present. Known canonical section names seen so far: `## Overview`, `## Architecture`, `## Components and Interfaces`, `## Testing strategy`. When authoring a new spec's design.md, use these exact `##` names up front to avoid the validation error.

### Kiro spec `tasks.md` requires four specific `##` section headings — the validator matches on heading text
The Kiro spec validator flags `tasks.md` with findings like "Missing recommended section: ## Overview / ## Tasks / ## Notes" and "Missing required section: ## Task Dependency Graph" when those exact `##`-level headings are absent — same behavior as the `design.md` validator (matches on literal heading text, not content presence). A tasks.md that jumps straight from the `# Implementation Plan:` title + blockquote into `## Task 0`, `## Task 1`, ... and ends with `## Sequencing & Dependencies` / `## Definition of Done` will fail, even though the content is all there.

Required layout (copy from a passing spec like `specs/done/cms-app-foundation/tasks.md`):
- `## Overview` — a short implementation-summary paragraph (can be distilled from the intro blockquote).
- `## Tasks` — a heading placed immediately before the first task. Under it, EACH TASK MUST be a checkbox list item, NOT a heading: `- [ ] N. Title` (e.g. `- [ ] 2. sqlc queries + repositories`), with all detail/test/`_Requirements:_`/`_Model:_` lines as indented sub-bullets (2 spaces; nest deeper bullets at 4). Do NOT use `## Task N` / `### Task N` headings for tasks — the validator rejects that (finding: "Tasks must use checkbox format (- [ ] N. Description), not ## or ### headings"). A per-task heading also silently ends the `## Tasks` section, pushing later tasks outside it. Only the task title line is a checkbox; sub-bullets are plain `-` (sub-task specs may instead use nested `- [ ] N.M` checkboxes, as in `cms-app-foundation`). Keep all four canonical `##` sections (`Overview`/`Tasks`/`Notes`/`Task Dependency Graph`) as the ONLY `##` headings — demote extras like "Definition of Done" to `###` under `## Notes` so no stray `##` interrupts the task list.
- `## Notes` — free-form notes; existing "Sequencing & Dependencies" content can be nested under it as `###`.
- `## Task Dependency Graph` — required; a ```json``` block with the `{ "waves": [{ "id": N, "tasks": [...] }] }` object format (see the separate dependency-graph note). Use each task's own id scheme as strings: sub-task specs use `"1.1"`, but flat specs whose checkboxes are numbered `- [ ] 0.`..`- [ ] 11.` use `"0"`,`"1"`,...`"11"`.

Fix is additive/re-leveling only — add the four headings, don't rewrite task content. `## Definition of Done` is an allowed extra section and does not conflict. When authoring a NEW tasks.md, include all four canonical headings up front. Sibling specs authored in the same batch (e.g. `admin-atm-management`, `admin-user-vendor-management`) tend to share the same missing-section defect — fix them together.
