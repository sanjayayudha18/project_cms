...
describe('FileUpload', () => {
  it('displays drop zone', () => {
    ...
    expect(
      screen.getByLabelText('Klik atau seret file untuk mengunggah'),
    ).toBeInTheDocument();
  });
  ...
  it('rejects files exceeding maxSizeBytes with appropriate error message', async () => {
    ...
    render(
      <FileUpload {...defaultProps} onFilesChange={onFilesChange} />,
    );
  ...
  it('rejects files with invalid type with appropriate error message', () => {
    ...
    expect(
      screen.getByText(/tidak didukung\. Gunakan JPEG, PNG, atau PDF/i),
    ).toBeInTheDocument();
    expect(onFilesChange).not.toHaveBeenCalled();
  });
  ...
  it('rejects files exceeding maxFiles count', async () => {
    ...
    expect(
      screen.getByText(/Maksimal 5 file per pengiriman/i),
    ).toBeInTheDocument();
    expect(onFilesChange).not.toHaveBeenCalled();
  });
  ...
  it('shows thumbnail preview for images', () => {
    ...
    render(
      <FileUpload {...defaultProps} files={[imageFile]} />,
    );
