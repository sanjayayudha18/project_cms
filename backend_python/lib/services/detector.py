...
Generic over whichever settings object is passed in -- each service's own
config.Settings implements not_processed_dir(file_type), input_dir(file_type),
file_pattern(file_type), and get_sla_time(file_type). No FILE_TYPES/FILE_PATTERNS
import here since those differ per service.
"""
...
class FileDetector:
    ...
    def scan_input_remaining(self, file_type: str) -> list[DetectedFile]:
        ...
        pattern = self.settings.file_pattern(file_type)
        if not dir_path.exists():
            return []
...
if __name__ == "__main__":
    ...
    # Uses a tiny duck-typed settings stand-in instead of any service's real
    # pydantic Settings, since this module no longer owns a concrete config.
    import tempfile
    from datetime import timedelta

    class _FakeSettings:
        def __init__(self, not_processed: Path):
            self._not_processed = not_processed

        def not_processed_dir(self, file_type: str) -> Path:
            return self._not_processed
    ...
    with tempfile.TemporaryDirectory() as tmp:
        settings = _FakeSettings(Path(tmp))
    ...
    settings = _FakeSettings(Path("."))
    late = LateDetector(settings)
    today = datetime.now(WIB).date()
    past = (datetime.now(WIB) - timedelta(hours=1)).time()

    class _PastSlaSettings(_FakeSettings):
        def get_sla_time(self, file_type: str) -> time:
            return past

    assert LateDetector(_PastSlaSettings(Path("."))).check_late(
        "dmaa", today, has_completed=False,
    ) is True
    assert LateDetector(_PastSlaSettings(Path("."))).check_late(
        "dmaa", today, has_completed=True,
    ) is False
