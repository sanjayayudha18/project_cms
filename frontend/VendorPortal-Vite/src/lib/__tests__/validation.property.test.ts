...
function validateFile(type: string, size: number, fileName: string): FileValidationResult {
...
// Generators
const validTypeArb = fc.constantFrom("image/jpeg", "image/png", "application/pdf");
...
describe("Property 9: File Upload Validation", () => {
  ...
  it('files with invalid type fail with "tidak didukung" error', () => {
    fc.assert(
      fc.property(invalidTypeArb, validSizeArb, fileNameArb, (type, size, name) => {
        const result = validateFile(type, size, name);
  ...
  it('files with size > 10MB fail with "melebihi batas" error', () => {
    fc.assert(
      fc.property(validTypeArb, invalidSizeArb, fileNameArb, (type, size, name) => {
        const result = validateFile(type, size, name);
        expect(result.valid).toBe(false);
  ...
  it("files with both invalid type AND size > 10MB fail validation (either error acceptable)", () => {
    fc.assert(
      fc.property(invalidTypeArb, invalidSizeArb, fileNameArb, (type, size, name) => {
        const result = validateFile(type, size, name);
        expect(result.valid).toBe(false);
