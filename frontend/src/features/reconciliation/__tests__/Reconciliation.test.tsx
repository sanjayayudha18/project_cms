...
describe('ReconciliationScreen - Column rendering', () => {
  ...
  it('renders page header with correct title and description', () => {
    ...
    expect(
      screen.getByText(/Bandingkan perhitungan kas fisik/),
    ).toBeInTheDocument();
  });
  ...
  it('renders the warning notice banner', () => {
    ...
    expect(
      screen.getByText(/exception tingkat tinggi belum terselesaikan/),
    ).toBeInTheDocument();
  });
});
...
describe('ReconciliationScreen - Empty state', () => {
  it('displays empty state message when data is empty array', async () => {
    ...
    const { ReconciliationScreen: EmptyScreen } = await import(
      '../ReconciliationScreen'
    );
    render(<EmptyScreen />);

    expect(
      screen.getByText('Tidak ada exception rekonsiliasi yang tersedia.'),
    ).toBeInTheDocument();
  });
