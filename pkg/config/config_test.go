...
func TestLoad_Success_WithDefaults(t *testing.T) {
	...
	if cfg.Port != "8080" {
		t.Errorf("Port = %q, want defaultPort 8080 when PORT unset", cfg.Port)
	}
}

func TestLoad_UsesCallerSuppliedDefaultPort(t *testing.T) {
	setEnvForTest(t)
	...
	if cfg.Port != "8081" {
		t.Errorf("Port = %q, want defaultPort 8081", cfg.Port)
	}
}
...
func TestLoad_PortEnvOverridesDefault(t *testing.T) {
	...
	if cfg.Port != "9090" {
		t.Errorf("Port = %q, want PORT env value 9090", cfg.Port)
	}
}
